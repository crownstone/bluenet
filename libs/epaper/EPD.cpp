...
void EPD_Class::frame_fn(const EPD_callback& cb, EPD_stage stage) {
    static uint8_t buffer[288 / 8]; // power of 2
    for (uint8_t line = 0; line < this->lines_per_display; ++line) {
        cb(buffer, line, this->bytes_per_line);
        this->line(line, buffer, 0, false, stage);
    }
}
...
void EPD_Class::frame_fn_repeat(const EPD_callback& cb, EPD_stage stage) {
    long stage_time = this->factored_stage_time;
    do {
        unsigned long t_start = millis();
        this->frame_fn(cb, stage);
        unsigned long t_end = millis();
        if (t_end > t_start) {
            stage_time -= t_end - t_start;
        } else {
...
void EPD_Class::line(uint16_t line, const uint8_t *data, uint8_t fixed_value, bool read_progmem, EPD_stage stage) {
	...
	for (uint16_t b = this->bytes_per_line; b > 0; --b) {
		if (0 != data) {
#if defined(__MSP430_CPU__) || defined(__MK20DX128__)

...
interface ID3Selection extends ID3Selectors {
    each ( func : (d:any,i:number) => any ) : ID3Selection;
    ...
    attrTween(name: string, interpolationFunction: (d:any,i:number,a:any) => any ) : ID3Selection;

    transition () : ID3Selection;

    duration( dur : number) : ID3Selection;
    ...
    data: {
        ...
        (values: (data: any, index: number) => any, index : (data: any, index: number) => any) : any;
    };

    on(str:string,callback: (s:any) => void) : ID3UpdateSelection;
...
interface ID3UpdateSelection extends ID3Selection {
    ...
    filter ( func : (d:any,i:number) => Boolean ) : ID3UpdateSelection;
}
...
//interface ID3TreeNode {
//    parent:ID3TreeNode;
//    children:ID3TreeNode[];
//    depth:number;
//    x:number;
//    y:number;
//}
//
//interface ID3TreeLink {
//
//}
//
//interface ID3TreeLayout {
//    (d : any[], index?: number): any[];
//    separation(accessor?: (ID3TreeNode,ID3TreeNode) => number) : ID3TreeLayout;
//    size(sizeArray: number[]) : ID3TreeLayout;
//    nodes(
//}


interface ID3TreeLayoutNode {
    parent: ID3TreeLayoutNode;
    children: ID3TreeLayoutNode[];
    depth: number;
    x: number;
    y: number;
}

interface ID3TreeLayoutLink {
    source: ID3TreeLayoutNode;
    target: ID3TreeLayoutNode;
}

interface ID3TreeLayout {
    sort(): (a: ID3TreeLayoutNode, b: ID3TreeLayoutNode) => number;
    sort(comparator: (a: ID3TreeLayoutNode, b: ID3TreeLayoutNode) => number): ID3TreeLayout;

    children(): (d: ID3TreeLayoutNode) => ID3TreeLayoutNode[];
    children(accessor: (d: ID3TreeLayoutNode) => ID3TreeLayoutNode[]): ID3TreeLayout;

    nodes(root: any): ID3TreeLayoutNode[];

    links(nodes: ID3TreeLayoutNode[]): ID3TreeLayoutLink[];

    separation(): (a: ID3TreeLayoutNode, b: ID3TreeLayoutNode) => number;
    separation(separation: (a: ID3TreeLayoutNode, b: ID3TreeLayoutNode) => number): ID3TreeLayout;

    size(): number[];
    size(s: number[]): ID3TreeLayout;
}

...
public:
	std::string _message;
	ble_exception(std::string message, std::string file = "<unknown>",
			int line = 0) :
			_message(message) {
	}
	~ble_exception() throw () {
...
#define BLE_CALL(function, args) do {uint32_t result = function args; APP_ERROR_CHECK(result); } while(0)
#define BLE_THROW_IF(result, message) do {if (result != NRF_SUCCESS) { std::string ble_error_message(message); LOG_DEBUG("BLE_THROW: %s", ble_error_message.c_str()); ble_error_handler(ble_error_message, __LINE__, __FILE__); } } while(0)
#define BLE_THROW(message) do { std::string ble_error_message(message); LOG_DEBUG("BLE_THROW: %s", ble_error_message.c_str()); ble_error_handler(ble_error_message, __LINE__, __FILE__); } while(0)
...
struct CharacteristicInit {
	...
	CharacteristicInit() :
			presentation_format( { }), char_md( { }), cccd_md( { }), attr_md(
					{ }) {
	}
};
...
public:
	...
	void setNotifyingEnabled(bool enabled) {
		LOG_DEBUG("[%s] notfying enabled: %s", _name.c_str(),
				enabled ? "true" : "false");
	 ...
	 */
	CharacteristicBase& setWritePermission(uint8_t securityMode,
...
protected:
	...
	virtual void configurePresentationFormat(
			ble_gatts_char_pf_t& presentation_format) {
...
template<typename T, typename E = void> class CharacteristicT: public Characteristic<
		T> {
public:
	CharacteristicT() :
			Characteristic<T>() {
...
template<typename T> class CharacteristicT<T,
		typename std::enable_if<std::is_base_of<ISerializable, T>::value>::type> : public Characteristic<
		T> {
public:
	CharacteristicT& operator=(const T& val) {
		Characteristic<T>::operator=(val);
...
template<typename T> class CharacteristicT<T,
		typename std::enable_if<std::is_arithmetic<T>::value>::type> : public Characteristic<
		T> {
public:
	CharacteristicT& operator=(const T& val) {
		Characteristic<T>::operator=(val);
...
template<> class CharacteristicT<std::string> : public Characteristic<
		std::string> {
public:
	...
	virtual uint16_t getValueMaxLength() {
		// Dominik: WTF why 25???? maximum possible is 512
		return 25;
	}

	virtual CharacteristicValue getCharacteristicValue() {
		return CharacteristicValue(getValue().length(),
				(uint8_t*) getValue().c_str());
	}
	virtual void setCharacteristicValue(const CharacteristicValue& value) {
...
template<> class CharacteristicT<CharacteristicValue> : public Characteristic<
		CharacteristicValue> {
public:
...
protected:
	...
	BLEStack* _stack;
	UUID _uuid;
	std::string _name;bool _primary;
	uint16_t _service_handle; // provided by stack.
	bool _started;

public:
	Service() :
			_name(""), _primary(true), _service_handle(BLE_CONN_HANDLE_INVALID), _started(
					false) {
	...
	virtual void on_connect(uint16_t conn_handle,
			ble_gap_evt_connected_t& gap_evt);  // FIXME NRFAPI  friend??

	virtual void on_disconnect(uint16_t conn_handle,
			ble_gap_evt_disconnected_t& gap_evt);  // FIXME NRFAPI
...
public:
	...
	virtual GenericService& addCharacteristic(
...
public:
	BatteryService() :
		...
		(*_characteristic).setUUID(UUID(BLE_UUID_BATTERY_LEVEL_CHAR)).setName(
				std::string("battery")).setDefaultValue(100);
...
public:
	...
	static const nrf_clock_lfclksrc_t defaultClockSource =
...
protected:
	...
	bool _inited;bool _started;bool _advertising;bool _scanning;
...
public:
	...
	Nrf51822BluetoothStack& setAdvertisingInterval(
	...
	Nrf51822BluetoothStack& setAdvertisingTimeoutSeconds(
			uint16_t advertisingTimeoutSeconds) {
		// TODO: stop advertising?
	...
	Nrf51822BluetoothStack& setMinConnectionInterval(
			uint16_t connectionInterval_1_25_ms) {
	...
	Nrf51822BluetoothStack& setMaxConnectionInterval(
			uint16_t connectionInterval_1_25_ms) {
	...
	Nrf51822BluetoothStack& setConnectionSupervisionTimeout(
			uint16_t conSupTimeout_10_ms) {
	...
	Nrf51822BluetoothStack& onDisconnect(
			const callback_disconnected_t& callback) {
	...
	Nrf51822BluetoothStack& onAdvertisement(
			const callback_advertisement_t& callback) {
	...
	bool isScanning();
#endif
	Nrf51822BluetoothStack& onRadioNotificationInterrupt(uint32_t distanceUs,
			callback_radio_t callback);

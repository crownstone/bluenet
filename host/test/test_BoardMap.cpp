...
void printBoardConfig(boards_config_t& config) {
    ...
    for (int i = 0; i < GAIN_COUNT; ++i) {
        std::cout << "  pinAinVoltageAfterLoad[" << i
                  << "]=" << (int)config.pinAinVoltageAfterLoad[i] << std::endl;
    }
    ...
    std::cout << "    hasAccuratePowerMeasurement=" << (int)config.flags.hasAccuratePowerMeasurement
              << std::endl;
    ...
    std::cout << "    dimmerOnWhenPinsFloat=" << (int)config.flags.dimmerOnWhenPinsFloat
              << std::endl;
    std::cout << "  deviceType=" << (int)config.deviceType << std::endl;

    for (int i = 0; i < GAIN_COUNT; ++i) {
        std::cout << "  voltageMultiplier[" << i << "]=" << (int)config.voltageMultiplier[i]
                  << std::endl;
    }
    for (int i = 0; i < GAIN_COUNT; ++i) {
        std::cout << "  voltageAfterLoadMultiplier[" << i
                  << "]=" << (int)config.voltageAfterLoadMultiplier[i] << std::endl;
    }
    for (int i = 0; i < GAIN_COUNT; ++i) {
        std::cout << "  currentMultiplier[" << i << "]=" << (int)config.currentMultiplier[i]
                  << std::endl;
    }
    for (int i = 0; i < GAIN_COUNT; ++i) {
    ...
    for (int i = 0; i < GAIN_COUNT; ++i) {
        std::cout << "  voltageAfterLoadOffset[" << i
                  << "]=" << (int)config.voltageAfterLoadOffset[i] << std::endl;
    }
    for (int i = 0; i < GAIN_COUNT; ++i) {
    ...
    std::cout << "  pwmTempVoltageThresholdDown=" << config.pwmTempVoltageThresholdDown
              << std::endl;
    std::cout << "  scanIntervalUs=" << config.scanIntervalUs << std::endl;
    std::cout << "  scanWindowUs=" << config.scanWindowUs << std::endl;
    std::cout << "  tapToToggleDefaultRssiThreshold=" << (int)config.tapToToggleDefaultRssiThreshold
              << std::endl;
}

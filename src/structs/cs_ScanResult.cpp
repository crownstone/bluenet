...
//		LOGw("_history[%d]: [%02X %02X %02X %02X %02X %02X]", i, _buffer->list[i].addr[5],
...
//			LOGw("replacing item at idx: %d", idx);
		}
		else {
			idx = _buffer->size++;
		}
		...
		if (idx >= 0) {
			LOGw("idx: %d, minRssi: %d", idx, minRssi);
			LOGw("NEW Advertisement from: [%02X %02X %02X %02X %02X %02X], rssi: %d", adrs_ptr[5],
					adrs_ptr[4], adrs_ptr[3], adrs_ptr[2], adrs_ptr[1],
					adrs_ptr[0], rssi);
...
//	}
//	else {
////		LOGw("Advertisement from: %s, rssi: %d, occ: %d", addrs, rssi, occ);
//	}
...
//				_buffer->list[i].addr[4], _buffer->list[i].addr[3], _buffer->list[i].addr[2], _buffer->list[i].addr[1],
//				_buffer->list[i].addr[0]);
		LOGw("[%02X %02X %02X %02X %02X %02X]\trssi: %d\tocc: %d", _buffer->list[i].addr[5],
				_buffer->list[i].addr[4], _buffer->list[i].addr[3], _buffer->list[i].addr[2], _buffer->list[i].addr[1],

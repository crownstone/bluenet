...
void CurrentCurve::init() {
	_payload = (uint8_t*)calloc(CURRENT_CURVE_SERIALIZED_SIZE, sizeof(uint8_t));
}

bool CurrentCurve::operator!=(const CurrentCurve &other) {
	if (_plength != other._plength) return true;
	if (memcmp(_payload, other._payload, _plength) != 0) return true;
	return false;
}

uint16_t CurrentCurve::getSerializedLength() const {
	return CURRENT_CURVE_HEADER_SIZE + _plength;
}

void CurrentCurve::setPayload(uint8_t *payload, uint16_t plength) {
	_plength = ((plength > CURRENT_CURVE_SERIALIZED_SIZE) ? CURRENT_CURVE_SERIALIZED_SIZE : plength);
	memcpy(_payload, payload, _plength);
}
...
void CurrentCurve::serialize(uint8_t* buffer, uint16_t length) const {
	if (getSerializedLength() < 3) return; // throw error
	
	uint8_t *ptr = buffer;
	*ptr++ = (_plength >> 8) & 0xFF;
	*ptr++ = _plength & 0xFF;
	if (_plength) memcpy(ptr, _payload, _plength);
}

void CurrentCurve::deserialize(uint8_t* buffer, uint16_t length) {
	if (getSerializedLength() < 3) return;

	uint8_t *ptr = buffer;
	_plength = (*ptr++ << 8) & 0xFF00;
	_plength |= *ptr++;
	memset(_payload, 0, CURRENT_CURVE_SERIALIZED_SIZE);
	_plength = ((_plength > CURRENT_CURVE_SERIALIZED_SIZE) ? CURRENT_CURVE_SERIALIZED_SIZE : _plength);
	memcpy(_payload, ptr, _plength);
}

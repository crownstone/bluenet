...
#define DM_TRC(...)  /**< Used for getting trace of execution in the module. */
...
static __INLINE api_result_t device_instance_free(uint32_t device_index)
{
    ...
    {
        DM_LOG("[DM]:[DI 0x%02X]: Freeing Instance.\r\n", device_index);
                                    ...
                                    uint32_t                  event_result)
{
    ...
    DM_LOG("[DM]: Notifying application of event 0x%02X\r\n", p_event->event_id);
    ...
    if (err_code == NRF_SUCCESS)
    {
        ...
        if (p_init_param->clear_persistent_data == false)
        {
            ...
            for (index = 0; index < DEVICE_MANAGER_MAX_BONDS; index++)
            {
                ...
                if (err_code == NRF_SUCCESS)
                {
                	DM_LOG("[DM]:[0x%02X]: Block handle 0x%08X.\r\n", index, block_handle.block_id);
...
api_result_t dm_security_setup_req(dm_handle_t * p_handle)
{
    ...
    DM_LOG("[DM]: << dm_security_setup_req, 0x%08X\r\n", err_code);
...
api_result_t dm_device_delete_all(dm_application_instance_t const * p_handle)
{
    ...
    DM_LOG("[DM]: >> dm_device_delete_all\r\n");
                                    ...
                                    dm_service_context_t const * p_context)
{
    ...
    DM_LOG("[DM]: >> dm_service_context_set\r\n");
                                    ...
                                    dm_service_context_t * p_context)
{
    ...
    if ((m_connection_table[p_handle->connection_id].state & STATE_CONNECTED) != STATE_CONNECTED)
    {
        DM_LOG("[DM]: Device must be connected to get context. \r\n");
    ...
    DM_LOG("[DM]: >> dm_service_context_get\r\n");
                                        ...
                                        dm_application_context_t const * p_context)
{
    ...
    DM_LOG("[DM]: >> dm_application_context_set\r\n");
                                        ...
                                        dm_application_context_t * p_context)
{
    ...
    DM_LOG("[DM]: >> dm_application_context_get\r\n");
...
api_result_t dm_application_context_delete(const dm_handle_t * p_handle)
{
    ...
    DM_LOG("[DM]: >> dm_application_context_delete\r\n");
                              ...
                              ble_gap_addr_t    * p_addr)
{
    ...
    if (p_handle->device_id == DM_INVALID_ID)
    {
        if ((p_handle->connection_id != DM_INVALID_ID) &&
            ...
            STATE_CONNECTED))
        {
            DM_LOG("[DM]:[CI 0x%02X]: Address get for non bonded active connection.\r\n",
                   p_handle->connection_id);
    ...
    else
    {
        ...
        {
            DM_LOG("[DM]:[DI 0x%02X]: Address get for bonded device.\r\n",
                   p_handle->device_id);
                                     ...
                                     ble_gap_sec_keyset_t * p_key_dist)
{
    ...
    DM_LOG("[DM]: >> dm_distributed_keys_get\r\n");

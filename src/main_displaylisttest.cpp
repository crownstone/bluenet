...
//    display.drawRect(50, 50, 100, 100, 1);
...
//    ri->~HuffmanImage();
    ...
    while(1) {
        ...
        int numPoints = 6;
        int x [] = { 22 , 150 , 150 , 60 + i * 4, 60 + i * 4 , 22 };
        int y [] = { 120, 120 , 40  , 40, 22 , 22 };

        for ( int index = 0; index < numPoints ; ++index ) {
            display.drawLine(x[index],y[index],x[(index+1)%numPoints],y[(index+1)%numPoints],1);
        }

//        float ddtheta = 3.14159 / 10.0f;
//        for ( float dtheta = 0.0f; dtheta < 3.14159f * 1.98f ; dtheta += ddtheta ) {
//            coord_t p0x = (coord_t) (centerX + (int)(cosf(theta+dtheta) * radius));
//            coord_t p0y = (coord_t) (centerY + (int)(sinf(theta+dtheta) * radius));
//            display.drawLine(centerX,centerY,p0x,p0y,1);
//
//            coord_t p1x = (coord_t) (centerX + (int)(cosf(theta+dtheta+ddtheta) * radius));
//            coord_t p1y = (coord_t) (centerY + (int)(sinf(theta+dtheta+ddtheta) * radius));
//
//            display.drawLine(p0x,p0y,p1x,p1y,1);
//        }
//        theta += 0.25f;

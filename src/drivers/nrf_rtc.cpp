...
uint32_t nrf_rtc_init(uint32_t seconds) {
	NRF_RTC0->PRESCALER = 1; // 12-bit prescaler for COUNTER frequency (32768/(PRESCALER+1)).
...
#if(NRF51_USE_SOFTDEVICE == 1)
	err_code = sd_nvic_ClearPendingIRQ(RTC0_IRQn);
...
#if(NRF51_USE_SOFTDEVICE == 1)
	err_code = sd_nvic_SetPriority(RTC0_IRQn, NRF_APP_PRIORITY_LOW);
	APP_ERROR_CHECK(err_code);
#else
	NVIC_SetPriority(RTC0_IRQn, NRF_APP_PRIORITY_LOW);
#endif

	log(DEBUG, "Tell to use RTC interrupt handler");
#if(NRF51_USE_SOFTDEVICE == 1)
	err_code = sd_nvic_EnableIRQ(RTC0_IRQn);
	APP_ERROR_CHECK(err_code);
#else
	...
	if (seconds) {
		NRF_RTC0->CC[0]    = seconds; // 24 bit counter --> 2^24 / (32768/(prescaler+1)) = 512 seconds to overflow

...
constexpr const char* operationModeName(OperationMode const& mode) {
	switch (mode) {
		case OperationMode::OPERATION_MODE_SETUP: return "SETUP";
		case OperationMode::OPERATION_MODE_DFU: return "DFU";
		case OperationMode::OPERATION_MODE_FACTORY_RESET: return "RESET";
		case OperationMode::OPERATION_MODE_NORMAL: return "NORMAL";
		case OperationMode::OPERATION_MODE_UNINITIALIZED: return "UNINIT";
	}
...
constexpr int ValidMode(OperationMode const& mode) {
	switch (mode) {
		...
		case OperationMode::OPERATION_MODE_UNINITIALIZED: return 1;
	}
	return 0;
}
...
struct cs_id_list_t {
	...
	cs_id_list_t(CS_TYPE type, std::vector<cs_state_id_t>* ids) : type(type), ids(ids) {}
};
...
public:
	 ...
	 * @param[in] mode            Indicates whether to get data from RAM, FLASH, FIRMWARE_DEFAULT, or a combination of
	 * this.
	 * @return                    Return code.
	 */
	 ...
	 *                                 There will be an event EVT_STORAGE_WRITE_DONE once it's actually written to
	 * flash.
	 ...
	 * @param[in] period         Period in seconds that throttling will be in effect. Must be smaller than
	 ...
	 * @return                    Return code.
	 */
	cs_ret_code_t remove(
			const CS_TYPE& type, cs_state_id_t id, const PersistenceMode mode = PersistenceMode::STRATEGY1);
	 ...
	 *
	 * TODO: implement
	 * Call this function after data has been changed via pointer. This makes sure the update will be propagated
	 * correctly.
	 *

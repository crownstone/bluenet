...
enum UartOpcodeRx {
	...
	//	UART_OPCODE_RX_ADC_CONFIG_SET =                   50101, // Set an adc channel config (payload:
	// uart_msg_adc_channel_config_t) 	UART_OPCODE_RX_ADC_CONFIG_SET_RANGE =             50102, // Set range of given
	// channel (payload: channel, range)
	...
	UART_OPCODE_RX_ADC_CONFIG_DIFFERENTIAL_CURRENT =
			50108,  // Enable differential mode on current channel (payload: bool enable)
	UART_OPCODE_RX_ADC_CONFIG_DIFFERENTIAL_VOLTAGE =
			50109,  // Enable differential mode on voltage channel (payload: bool enable)
	...
	UART_OPCODE_RX_POWER_LOG_FILTERED_CURRENT =
			50202,  // Enable writing filtered current samples (payload: bool enable)
	//	UART_OPCODE_RX_POWER_LOG_FILTERED_VOLTAGE =       50203, // Enable writing filtered voltage samples (payload:
	// bool enable)
...
enum UartOpcodeTx {
	...
	UART_OPCODE_TX_ERR_REPLY_PARSING_FAILED =
			9900,  // Replied on error, the command probably has a wrong format, or is too large.
	...
	//	UART_OPCODE_TX_DECRYPTION_FAILED =                10003, // Sent when decryption failed due to missing or wrong
	// key.
	UART_OPCODE_TX_PRESENCE_CHANGE = 10004,  // Sent when the presence has changed, payload: presence_change_t. When the
											 // first user enters, multiple msgs will be sent.
	...
	UART_OPCODE_TX_MESH_STATE_PART_0 =
			10103,  // Received part of state of external stone, payload: cs_mesh_model_msg_state_0_t
	UART_OPCODE_TX_MESH_STATE_PART_1 =
			10104,  // Received part of state of external stone, payload: cs_mesh_model_msg_state_1_t
	UART_OPCODE_TX_MESH_RESULT =
			10105,  // Received the result of a mesh command, payload: uart_msg_mesh_result_packet_header_t + data.
	UART_OPCODE_TX_MESH_ACK_ALL_RESULT = 10106,  // Whether all stone IDs were acked, payload: result_packet_header_t.
	UART_OPCODE_TX_RSSI_DATA_MESSAGE = 10107,  // When MeshTopologyResearch receives an rssi_data_message, it is sent to
											   // host. Payload: RssiDataMessage.
	UART_OPCODE_TX_ASSET_INFO_MAC =
			10108,  // Payload: cs_asset_info_mac_t. Info about an asset a Crownstone on the mesh has forwarded.

	UART_OPCODE_TX_NEIGHBOUR_RSSI = 10111,  // Payload: mesh_topology_neighbour_rssi_t
	UART_OPCODE_TX_ASSET_INFO_ID =
			10112,  // Payload: cs_asset_info_id_t. Info about an asset a Crownstone on the mesh has forwarded.

	UART_OPCODE_TX_LOG           = 10200,  // Debug logs, payload is in the form: [uart_msg_log_header_t,
										   // [uart_msg_log_arg_header_t, data], [uart_msg_log_arg_header_t, data], ...]
	UART_OPCODE_TX_LOG_ARRAY     = 10201,  // Debug logs, payload is in the form: [uart_msg_log_header_t,
										   // [uart_msg_log_arg_header_t, data], [uart_msg_log_arg_header_t, data], ...]

	////////// Developer messages in release builds. //////////
	UART_OPCODE_TX_EVT           = 40000,  // Send internal events, this protocol may change

	//	UART_OPCODE_TX_MESH_STATE_TIME =                  40102, // Received time of other crownstone, payload:
	...
	UART_OPCODE_TX_MESH_PROFILE_LOCATION =
			40110,  // Received the location of a profile, payload: cs_mesh_model_msg_profile_location_t.
	UART_OPCODE_TX_MESH_SET_BEHAVIOUR_SETTINGS =
			40111,  // Received cmd to set behaviour settings, payload: behaviour_settings_t
	UART_OPCODE_TX_MESH_TRACKED_DEVICE_REGISTER =
			40112,  // Received cmd to register a tracked device, payload: cs_mesh_model_msg_device_register_t
	UART_OPCODE_TX_MESH_TRACKED_DEVICE_TOKEN =
			40113,  // Received cmd to set the token of a tracked device, payload: cs_mesh_model_msg_device_token_t
	...
	UART_OPCODE_TX_MESH_TRACKED_DEVICE_HEARTBEAT =
			40120,  // Received heartbeat cmd of a tracked device, payload: cs_mesh_model_msg_device_heartbeat_t
...
constexpr bool mustBeEncryptedTx(UartOpcodeTx opCode) {
	switch (opCode) {
		...
		case UartOpcodeTx::UART_OPCODE_TX_BOOTED: return false;
		default:
			if (opCode >= 50000) {
				return false;
			}

...
public:
	 ...
	 *
	 * Must be done after the radio has been initialized.
...
private:
	...
	/**
	 * Whether the advertisement data should be advertised.
	 * When true, the advertisement data buffers are also allocated.
	 */
	bool                                        _includeAdvertisementData = false;

	/**
	 * Whether the scan response data should be advertised.
	 * When true, the advertisement data buffers are also allocated.
	 */
	...
	/**
	 * Set the cached TX power at the soft device.
	 */
	void updateTxPower();

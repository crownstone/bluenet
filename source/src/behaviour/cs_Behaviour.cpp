...
Behaviour::SerializedDataType Behaviour::serialize() {
	...
	resultIter      = std::copy_n(
            std::begin(WireFormat::serialize(static_cast<uint8_t>(typ))), WireFormat::size<uint8_t>(), resultIter);
	resultIter =
			std::copy_n(std::begin(WireFormat::serialize(activeIntensity)), WireFormat::size<uint8_t>(), resultIter);
	...
	resultIter = std::copy_n(
			std::begin(WireFormat::serialize(behaviourAppliesFrom)), WireFormat::size<TimeOfDay>(), resultIter);
	resultIter = std::copy_n(
			std::begin(WireFormat::serialize(behaviourAppliesUntil)), WireFormat::size<TimeOfDay>(), resultIter);

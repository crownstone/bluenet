...
void UartCommandHandler::handleCommand(
	...
	switch (opCode) {
		case UART_OPCODE_RX_HELLO: handleCommandHello(commandData); break;
		case UART_OPCODE_RX_SESSION_NONCE: handleCommandSessionNonce(commandData); break;
		case UART_OPCODE_RX_HEARTBEAT: handleCommandHeartBeat(commandData, wasEncrypted); break;
		case UART_OPCODE_RX_STATUS: handleCommandStatus(commandData); break;
		case UART_OPCODE_RX_GET_MAC: handleCommandGetMacAddress(commandData); break;
		case UART_OPCODE_RX_CONTROL: handleCommandControl(commandData, source, accessLevel, resultBuffer); break;
		...
		case UART_OPCODE_RX_ENABLE_ADVERTISEMENT:
			...
			break;
		case UART_OPCODE_RX_ENABLE_MESH: dispatchEventForCommand(CS_TYPE::CMD_ENABLE_MESH, commandData); break;
		case UART_OPCODE_RX_GET_ID: handleCommandGetId(commandData); break;
		...
		case UART_OPCODE_RX_POWER_LOG_FILTERED_CURRENT:
			...
			break;
		case UART_OPCODE_RX_POWER_LOG_POWER: dispatchEventForCommand(CS_TYPE::CMD_ENABLE_LOG_POWER, commandData); break;

		case UART_OPCODE_RX_INJECT_EVENT: handleCommandInjectEvent(commandData); break;
...
EncryptionAccessLevel UartCommandHandler::getRequiredAccessLevel(const UartOpcodeRx opCode) {
	...
	switch (opCode) {
		...
		case UART_OPCODE_RX_CONTROL: return EncryptionAccessLevel::MEMBER;

		default: LOGw("Unknown opcode: %i", opCode); return EncryptionAccessLevel::NO_ONE;
	}
}

void UartCommandHandler::dispatchEventForCommand(CS_TYPE type, cs_data_t commandData) {
...
void UartCommandHandler::handleCommandControl(
		cs_data_t commandData,
	...
	UartHandler::getInstance().writeMsgStart(
			UART_OPCODE_TX_CONTROL_RESULT, sizeof(resultHeader) + resultHeader.payloadSize);
	UartHandler::getInstance().writeMsgPart(
			UART_OPCODE_TX_CONTROL_RESULT, (uint8_t*)&resultHeader, sizeof(resultHeader));
	UartHandler::getInstance().writeMsgPart(
			UART_OPCODE_TX_CONTROL_RESULT, event.result.buf.data, event.result.dataSize);
	UartHandler::getInstance().writeMsgEnd(UART_OPCODE_TX_CONTROL_RESULT);
}

void UartCommandHandler::handleCommandHubDataReply(
		cs_data_t commandData,
		const cmd_source_with_counter_t source,
		const EncryptionAccessLevel accessLevel,
		cs_data_t resultBuffer) {
	LOGd("handleCommandHubDataReply");
	uart_msg_hub_data_reply_header_t* replyHeader =
			reinterpret_cast<uart_msg_hub_data_reply_header_t*>(commandData.data);

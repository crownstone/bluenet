...
void CrownstoneService::addControlCharacteristic(
	...
	_controlCharacteristic->onWrite(
			[&](const EncryptionAccessLevel accessLevel, const buffer_ptr_t& value, uint16_t length) -> void {
				...
				// At this point it is too late to check if writeBuffer was locked, because the softdevice doesn't care
				// if the writeBuffer was locked, it writes to the buffer in any case.

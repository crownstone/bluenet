...
void TestCrownstoneCentral::writeGetMac() {
	TYPIFY(CMD_CS_CENTRAL_WRITE) cmdPacket = {.commandType = CTRL_CMD_GET_MAC_ADDRESS, .data = cs_data_t()};
...
void TestCrownstoneCentral::writeSetup() {
	...
	State::getInstance().get(
			CS_TYPE::CONFIG_KEY_SERVICE_DATA, &(setupData.serviceDataKey), sizeof(setupData.serviceDataKey));
	State::getInstance().get(
			CS_TYPE::CONFIG_KEY_LOCALIZATION, &(setupData.localizationKey), sizeof(setupData.localizationKey));
	...
	switch (event.type) {
		...
		case CS_TYPE::EVT_CS_CENTRAL_WRITE_RESULT: {
			...
			if (result->result.isInitialized()) {
				_log(SERIAL_INFO,
					 false,
					 "Reply: protocol=%u type=%u result=%u data=",

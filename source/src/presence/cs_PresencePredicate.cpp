...
bool PresencePredicate::isTrue(PresenceStateDescription presence) {
	switch (_condition) {
		...
		case Condition::AnyoneInSelectedRooms:
			return (presence.getBitmask() & _presence.getBitmask()) != 0;
		case Condition::NooneInSelectedRooms:
			return (presence.getBitmask() & _presence.getBitmask()) == 0;
		case Condition::AnyoneInSphere:
			return presence.getBitmask() != 0;
...
PresencePredicate::SerializedDataType PresencePredicate::serialize() {
	...
	std::copy_n(std::begin(WireFormat::serialize(static_cast<uint64_t>(_presence.getBitmask()))),     8, std::begin(result) + 1);
	return result;
}

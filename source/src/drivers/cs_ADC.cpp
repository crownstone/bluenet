...
#if defined(TEST_PIN_ZERO_CROSS) || defined(TEST_PIN_START) || defined(TEST_PIN_STOP) || defined(TEST_PIN_PROCESS) \
		|| defined(TEST_PIN_INT_END) || defined(TEST_PIN_SAMPLE)
#ifdef DEBUG
...
ADC::ADC() : _bufferQueue(CS_ADC_NUM_BUFFERS), _saadcBufferQueue(CS_ADC_NUM_SAADC_BUFFERS) {
...
cs_ret_code_t ADC::init(const adc_config_t& config) {
	...
	nrf_gpiote_task_configure(
			CS_ADC_GPIOTE_CHANNEL_START, TEST_PIN_SAMPLE, NRF_GPIOTE_POLARITY_TOGGLE, NRF_GPIOTE_INITIAL_VALUE_LOW);
...
cs_ret_code_t ADC::initChannel(adc_channel_id_t channel, adc_channel_config_t& config) {
	LOGi("Init channel %u on AIN%u, range=%umV, ref=ain%u",
		 channel,
		 config.pin,
		 config.rangeMilliVolt,
		 config.referencePin);
	...
	switch (CS_ADC_RESOLUTION) {
		case NRF_SAADC_RESOLUTION_8BIT: adcBits = 8; break;
		case NRF_SAADC_RESOLUTION_10BIT: adcBits = 10; break;
		case NRF_SAADC_RESOLUTION_12BIT: adcBits = 12; break;
		case NRF_SAADC_RESOLUTION_14BIT: adcBits = 14; break;
	}
...
void ADC::stop() {
	LOGAdcDebug("stop");
	switch (_state) {
		case ADC_STATE_IDLE: LOGw("already stopped"); return;
		default: break;
	}
...
void ADC::start() {
	...
	switch (_state) {
		...
		case ADC_STATE_BUSY: LOGw("already started"); return;
		default: break;
	}
...
cs_ret_code_t ADC::_fillSaadcQueue(bool fromInterrupt) {
	...
	switch (_saadcState) {
		...
		case ADC_SAADC_STATE_BUSY: break;
	}
...
void ADC::_handleAdcInterrupt() {
	if (nrf_saadc_event_check(NRF_SAADC_EVENT_END)) {
		...
		LOGAdcInterruptDebug(
				"Done bufIndex=%u queueSize=%u nextBuf=%u",
				bufIndex,
				_saadcBufferQueue.size(),
				_saadcBufferQueue.peek());
...
nrf_ppi_channel_t ADC::getPpiChannel(uint8_t index) {
	switch (index) {
		...
		case 16: return NRF_PPI_CHANNEL16;
	}
...
nrf_saadc_input_t ADC::getAdcPin(const adc_pin_id_t pinNum) {
	switch (pinNum) {
		...
		case CS_ADC_PIN_VDD: return NRF_SAADC_INPUT_VDD;
		default: assert(false, "invalid adc pin"); return NRF_SAADC_INPUT_DISABLED;
	}
}

nrf_saadc_event_t ADC::getLimitLowEvent(adc_channel_id_t channel) {
	switch (channel) {
		case 0: return NRF_SAADC_EVENT_CH0_LIMITL;
		case 1: return NRF_SAADC_EVENT_CH1_LIMITL;
		case 2: return NRF_SAADC_EVENT_CH2_LIMITL;
		case 3: return NRF_SAADC_EVENT_CH3_LIMITL;
		case 4: return NRF_SAADC_EVENT_CH4_LIMITL;
		case 5: return NRF_SAADC_EVENT_CH5_LIMITL;
		case 6: return NRF_SAADC_EVENT_CH6_LIMITL;
		case 7: return NRF_SAADC_EVENT_CH7_LIMITL;
	}
...
nrf_saadc_event_t ADC::getLimitHighEvent(adc_channel_id_t channel) {
	switch (channel) {
		case 0: return NRF_SAADC_EVENT_CH0_LIMITH;
		case 1: return NRF_SAADC_EVENT_CH1_LIMITH;
		case 2: return NRF_SAADC_EVENT_CH2_LIMITH;
		case 3: return NRF_SAADC_EVENT_CH3_LIMITH;
		case 4: return NRF_SAADC_EVENT_CH4_LIMITH;
		case 5: return NRF_SAADC_EVENT_CH5_LIMITH;
		case 6: return NRF_SAADC_EVENT_CH6_LIMITH;
		case 7: return NRF_SAADC_EVENT_CH7_LIMITH;
	}

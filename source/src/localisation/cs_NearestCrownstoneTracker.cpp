...
void NearestCrownstoneTracker::handleMeshMsgEvent(event_t& evt) {
	...
	if (meshMsgEvent->type == CS_MESH_MODEL_TYPE_ASSET_INFO_ID) {
		...
		onReceiveAssetReport(meshMsgEvent->getPacket<CS_MESH_MODEL_TYPE_ASSET_INFO_ID>(), meshMsgEvent->srcAddress);

		evt.result = ERR_SUCCESS;
	}
}

bool NearestCrownstoneTracker::handleAcceptedAsset(
		const scanned_device_t& asset, const asset_id_t& id, uint8_t filterBitmask) {
	LOGNearestCrownstoneTrackerVerbose("handleAcceptedAsset");
	auto assetMsg = cs_mesh_model_msg_asset_report_id_t{
			.id = id, .filterBitmask = filterBitmask, .rssi = asset.rssi, .channel = 0};
...
bool NearestCrownstoneTracker::onReceiveAssetAdvertisement(cs_mesh_model_msg_asset_report_id_t& incomingReport) {
	LOGNearestCrownstoneTrackerVerbose(
			"onReceiveAssetAdvertisement myId(%u), report(%d dB ch.%u)",
			_myStoneId,
			incomingReport.rssi,
			decompressChannel(incomingReport.channel));
	...
	auto incomingRssiAndChannelCompressed =
			rssi_and_channel_t(incomingReport.rssi, decompressChannel(incomingReport.channel));
	auto incomingRssiAndChannel = incomingRssiAndChannelCompressed.toFloat();

	auto recordedNearestRssiWithFallOff =
			rssi_and_channel_float_t(record.nearestRssi)
					.fallOff(
							RSSI_FALL_OFF_RATE_DB_PER_S,
							record.lastReceivedCounter * 1e-3f * AssetStore::LAST_RECEIVED_COUNTER_PERIOD_MS);
...
void NearestCrownstoneTracker::onReceiveAssetReport(
		cs_mesh_model_msg_asset_report_id_t& incomingReport, stone_id_t reporter) {
	LOGNearestCrownstoneTrackerVerbose(
			"onReceive witness report, myId(%u), reporter(%u), rssi(%i #%u)",
			_myStoneId,
			reporter,
			incomingReport.rssi,
			decompressChannel(incomingReport.channel));
	...
	if (reporter == record.nearestStoneId) {
		...
		if (record.nearestStoneId == 0 || incomingRssiAndChannel.isCloserThan(recordedNearestRssiWithFallOff)) {
			LOGNearestCrownstoneTrackerVerbose(
					"Received a witnessreport from another crownstone that is better than my winner.");
			saveWinningReport(record, incomingRssiAndChannelCompressed, reporter);
			sendUartUpdate(record);
...
void NearestCrownstoneTracker::sendUartUpdate(asset_record_t& record) {
	auto uartMsg = asset_report_uart_id_t{
			.assetId = record.assetId, .stoneId = record.nearestStoneId, .rssi = record.nearestRssi, .channel = 0};

	UartHandler::getInstance().writeMsg(
			UartOpcodeTx::UART_OPCODE_TX_ASSET_INFO_ID, reinterpret_cast<uint8_t*>(&uartMsg), sizeof(uartMsg));
}
...
void NearestCrownstoneTracker::saveWinningReport(
		asset_record_t& rec, rssi_and_channel_t winningRssi, stone_id_t winningId) {

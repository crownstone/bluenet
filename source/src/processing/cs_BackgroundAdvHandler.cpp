...
void BackgroundAdvertisementHandler::parseServicesAdvertisement(scanned_device_t* scannedDevice) {
	...
	for (uint8_t i = 0; i < serviceUuids.len / 2; ++i) {
		LOGBackgroundAdvVerbose(
				"uuid=%u %u (0x%02X 0x%02X)",
				serviceUuids.data[2 * i + 0],
				serviceUuids.data[2 * i + 1],
				serviceUuids.data[2 * i + 0],
				serviceUuids.data[2 * i + 1]);
	...
	LOGBackgroundAdvVerbose(
			"store bitmask 0x%08X%08X 0x%08X%08X",
			(uint32_t)(_lastBitmask[0] >> 32),
			(uint32_t)(_lastBitmask[0]),
			(uint32_t)(_lastBitmask[1] >> 32),
			(uint32_t)(_lastBitmask[1]));
}
...
void BackgroundAdvertisementHandler::parseAdvertisement(scanned_device_t* scannedDevice) {
	...
	uint64_t left = ((uint64_t)servicesMask[0] << (7 * 8)) + ((uint64_t)servicesMask[1] << (6 * 8))
					+ ((uint64_t)servicesMask[2] << (5 * 8)) + ((uint64_t)servicesMask[3] << (4 * 8))
					+ ((uint64_t)servicesMask[4] << (3 * 8)) + ((uint64_t)servicesMask[5] << (2 * 8))
					+ ((uint64_t)servicesMask[6] << (1 * 8)) + ((uint64_t)servicesMask[7] << (0 * 8));
	uint64_t right = ((uint64_t)servicesMask[0 + 8] << (7 * 8)) + ((uint64_t)servicesMask[1 + 8] << (6 * 8))
					 + ((uint64_t)servicesMask[2 + 8] << (5 * 8)) + ((uint64_t)servicesMask[3 + 8] << (4 * 8))
					 + ((uint64_t)servicesMask[4 + 8] << (3 * 8)) + ((uint64_t)servicesMask[5 + 8] << (2 * 8))
					 + ((uint64_t)servicesMask[6 + 8] << (1 * 8)) + ((uint64_t)servicesMask[7 + 8] << (0 * 8));
	LOGBackgroundAdvVerbose(
			"left=0x%08X%08X right=0x%08X%08X",
			(uint32_t)(left >> 32),
			(uint32_t)(left),
			(uint32_t)(right >> 32),
			(uint32_t)(right));
	...
	if (memcmp(_lastMacAddress, scannedDevice->address, MAC_ADDRESS_LEN) == 0) {
		...
		LOGBackgroundAdvVerbose(
				"Use last bitmask left=0x%08X%08X right=0x%08X%08X",
				(uint32_t)(left >> 32),
				(uint32_t)(left),
				(uint32_t)(right >> 32),
				(uint32_t)(right));
	...
	uint64_t part2  = ((left & 0x3FFFFF) << 20)
					 | ((right >> (64 - 20))
						& 0x0FFFFF);  // Last 64-42=22 bits from left, and first 42−(64−42)=20 bits from right.
	...
	LOGBackgroundAdvVerbose(
			"part1=0x%08X%08X part2=0x%08X%08X part3=0x%08X%08X result=0x%08X%08X",
			(uint32_t)(part1 >> 32),
			(uint32_t)(part1),
			(uint32_t)(part2 >> 32),
			(uint32_t)(part2),
	...
	if (protocol == 1) {
		...
		LOGBackgroundAdvDebug(
				"v1 token=[%u %u %u]", parsed.deviceToken[0], parsed.deviceToken[1], parsed.deviceToken[2]);
	...
	if (backgroundAdvertisement.protocol != 0 || backgroundAdvertisement.sphereId != _sphereId) {
		LOGBackgroundAdvVerbose(
				"wrong protocol (%u) or sphereId (%u vs %u)",
				backgroundAdvertisement.protocol,
				backgroundAdvertisement.sphereId,
				_sphereId);
		return;
	}
	...
	bool success = RC5::getInstance().decrypt(
			encryptedPayload, sizeof(encryptedPayload), decryptedPayload, sizeof(decryptedPayload));
	if (!success) {
		return;
	}
	...
	if ((decryptedPayload[0] == timestampRounded - 1) || (decryptedPayload[0] == timestampRounded)
...
void BackgroundAdvertisementHandler::handleBackgroundAdvertisement(adv_background_t* backgroundAdvertisement) {
	...
	_log(SERIAL_DEBUG,
		 false,
		 "protocol=%u sphereId=%u rssi=%i ",
		 backgroundAdvertisement->protocol,
		 backgroundAdvertisement->sphereId,
		 backgroundAdvertisement->rssi);
	...
	LOGBackgroundAdvDebug(
			"validation=%u locationId=%u profileId=%u rssiOffset=%u flags=%u rssi=%i adjusted_rssi=%i",
			decryptedPayload[0],
			parsed.locationId,
			parsed.profileId,
			rssiOffset,

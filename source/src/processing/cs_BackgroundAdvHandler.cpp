...
void BackgroundAdvertisementHandler::handleBackgroundAdvertisement(adv_background_t* backgroundAdvertisement) {
	...
	_log(SERIAL_DEBUG, false, false, "protocol=%u sphereId=%u rssi=%i ", backgroundAdvertisement->protocol, backgroundAdvertisement->sphereId, backgroundAdvertisement->rssi);
	_log(SERIAL_DEBUG, false, false, "payload=[%u %u] address=", decryptedPayload[0], decryptedPayload[1]);

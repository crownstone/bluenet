...
void TapToToggle::init(int8_t defaultRssiThreshold) {
	...
	State::getInstance().get(
			CS_TYPE::CONFIG_TAP_TO_TOGGLE_RSSI_THRESHOLD_OFFSET, &thresholdOffset, sizeof(thresholdOffset));
...
void TapToToggle::handleBackgroundAdvertisement(adv_background_parsed_t* adv) {
	LOGT2Td("addr=%x:%x:%x:%x:%x:%x rssi=%i flags=%u",
			adv->macAddress[0],
			adv->macAddress[1],
			adv->macAddress[2],
			adv->macAddress[3],
	...
	// By placing this check here, the score will drop, which will make it more likely to trigger again when phone is
	// kept close.
	if (timeoutCounter != 0) {
...
void TapToToggle::handleEvent(event_t& event) {
	switch (event.type) {
		...
		case CS_TYPE::CONFIG_TAP_TO_TOGGLE_RSSI_THRESHOLD_OFFSET: {
			TYPIFY(CONFIG_TAP_TO_TOGGLE_RSSI_THRESHOLD_OFFSET)
			thresholdOffset = *((TYPIFY(CONFIG_TAP_TO_TOGGLE_RSSI_THRESHOLD_OFFSET)*)event.data);
			rssiThreshold   = defaultRssiThreshold + thresholdOffset;
			break;
		}
		default: break;

...
void CommandAdvHandler::parseAdvertisement(scanned_device_t* scannedDevice) {
	...
	errCode = CsUtils::findAdvType(
			BLE_GAP_AD_TYPE_16BIT_SERVICE_UUID_COMPLETE, scannedDevice->data, scannedDevice->dataSize, &services16bit);
	...
	if (scannedDevice->rssi > RSSI_LOG_THRESHOLD) {
		...
		_logArray(
				...
				true,
				services128bit.data,
				services128bit.len);  // Received as uint128, so bytes are reversed.
#endif
	}
	...
	for (int i = 0; i < CMD_ADV_NUM_SERVICES_16BIT; ++i) {
		...
		switch (sequence) {
			...
			case 1:
				...
				encryptedPayloadRC5[0] =
						((serviceUuid >> (16 - 2 - 2 - 8 - 4)) & 0x0F)
						<< (16 - 4);  // Last 4 bits of this service UUID are first 4 bits of encrypted payload[0].
				memcpy(nonce + 2, &serviceUuid, sizeof(uint16_t));
				break;
			case 2:
				//			header.sequence2 = sequence;
				encryptedPayloadRC5[0] +=
						((serviceUuid >> (16 - 2 - 12)) & 0x0FFF)
						<< (16 - 4 - 12);  // Bits 2 - 13 of this service UUID are last 12 bits of encrypted payload[0].
				encryptedPayloadRC5[1] =
						((serviceUuid >> (16 - 2 - 12 - 2)) & 0x03)
						<< (16 - 2);  // Last 2 bits of this service UUID are first 2 bits of encrypted payload[1].
				memcpy(nonce + 4, &serviceUuid, sizeof(uint16_t));
				break;
			case 3:
				//			header.sequence3 = sequence;
				encryptedPayloadRC5[1] +=
						((serviceUuid >> (16 - 2 - 14)) & 0x3FFF)
						<< (16 - 2
							- 14);  // Last 14 bits of this service UUID are last 14 bits of encrypted payload[1].
	...
	bool validated = handleEncryptedCommandPayload(
			scannedDevice, header, nonceData, services128bit, encryptedPayloadRC5, decryptedPayloadRC5);
	if (validated) {
...
int CommandAdvHandler::checkSimilarCommand(
	...
	for (int i = 0; i < CMD_ADV_MAX_CLAIM_COUNT; ++i) {
		if (_claims[i].deviceToken == deviceToken) {
			if (_claims[i].timeoutCounter && _claims[i].encryptedRC5 == encryptedRC5
				&& memcmp(_claims[i].encryptedData, encryptedData.data, CMD_ADC_ENCRYPTED_DATA_SIZE) == 0) {
...
bool CommandAdvHandler::claim(
		uint8_t deviceToken, cs_data_t& encryptedData, uint16_t encryptedRC5, uint16_t decryptedRC5, int index) {
	assert(encryptedData.len == CMD_ADC_ENCRYPTED_DATA_SIZE, "Invalid size");
...
bool CommandAdvHandler::handleEncryptedCommandPayload(
		...
		uint16_t decryptedPayloadRC5[2]) {
	int claimIndex =
			checkSimilarCommand(header.deviceToken, encryptedPayload, encryptedPayloadRC5[1], decryptedPayloadRC5[1]);
	if (claimIndex == -2) {
	...
	switch (header.accessLevel) {
		case 0: accessLevel = ADMIN; break;
		case 1: accessLevel = MEMBER; break;
		case 2: accessLevel = BASIC; break;
		case 4: accessLevel = SETUP; break;
	...
	uint32_t validationTimestamp =
			(decryptedData[0] << 0) | (decryptedData[1] << 8) | (decryptedData[2] << 16) | (decryptedData[3] << 24);
	...
	cmd_source_with_counter_t source(
			cmd_source_t(CS_CMD_SOURCE_TYPE_BROADCAST, header.deviceToken), (decryptedPayloadRC5[0] >> 8) & 0xFF);
	...
	switch (type) {
		...
		case ADV_CMD_MULTI_SWITCH: {
			...
			LOGCommandAdvDebug(
					"send cmd type=%u source: type=%u id=%u count=%u",
					controlCmd.type,
					source.source.type,
					source.source.id,
					source.count);
		...
		case ADV_CMD_SET_TIME: {
			...
			if (CsUtils::isBitSet(flags, 1)) {
				sun_time_t sunTime;
				sunTime.sunrise = (commandData[flagsSize + setTimeSize + 0] << 0)
								  + (commandData[flagsSize + setTimeSize + 1] << 8)
								  + (commandData[flagsSize + setTimeSize + 2] << 16);
				sunTime.sunset = (commandData[flagsSize + setTimeSize + 3] << 0)
								 + (commandData[flagsSize + setTimeSize + 4] << 8)
								 + (commandData[flagsSize + setTimeSize + 5] << 16);
		...
		default: LOGd("Unkown adv cmd: %u", type); return true;
	}
	return true;
}
...
bool CommandAdvHandler::decryptRC5Payload(uint16_t encryptedPayload[2], uint16_t decryptedPayload[2]) {
	...
	bool success = RC5::getInstance().decrypt(
			encryptedPayload,
			sizeof(uint16_t) * 2,
			decryptedPayload,
			sizeof(uint16_t) * 2);  // Can't use sizeof(encryptedPayload) as that returns size of pointer.
	LOGCommandAdvVerbose("decrypted RC5=[%u %u]", decryptedPayload[0], decryptedPayload[1]);
	return success;
}

void CommandAdvHandler::handleDecryptedRC5Payload(
	...
	switch (header.protocol) {
		case 0: backgroundAdv.protocol = 0; break;
		default: return;
	}
...
EncryptionAccessLevel CommandAdvHandler::getRequiredAccessLevel(const AdvCommandTypes type) {
	switch (type) {
		case ADV_CMD_NOOP:
		case ADV_CMD_MULTI_SWITCH:
		case ADV_CMD_UPDATE_TRACKED_DEVICE: return BASIC;
		case ADV_CMD_SET_TIME:
		case ADV_CMD_SET_BEHAVIOUR_SETTINGS: return MEMBER;
		case ADV_CMD_UNKNOWN: return NOT_SET;
	}
	return NOT_SET;
}

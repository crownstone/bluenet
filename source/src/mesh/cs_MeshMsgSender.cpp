...
cs_ret_code_t MeshMsgSender::sendMultiSwitchItem(
		const internal_multi_switch_item_t* switchItem,
...
cs_ret_code_t MeshMsgSender::sendProfileLocation(
		const cs_mesh_model_msg_profile_location_t* packet, uint8_t transmissions) {
...
cs_ret_code_t MeshMsgSender::sendTrackedDeviceRegister(
		const cs_mesh_model_msg_device_register_t* packet, uint8_t transmissions) {
	LOGd("sendTrackedDeviceRegister id=%u profile=%u location=%u",
		 packet->deviceId,
		 packet->profileId,
		 packet->locationId);
...
cs_ret_code_t MeshMsgSender::sendTrackedDeviceToken(
		const cs_mesh_model_msg_device_token_t* packet, uint8_t transmissions) {
	LOGd("sendTrackedDeviceToken id=%u TTL=%u token=%u %u %u",
		 packet->deviceId,
		 packet->ttlMinutes,
...
cs_ret_code_t MeshMsgSender::sendTrackedDeviceHeartbeat(
		const cs_mesh_model_msg_device_heartbeat_t* packet, uint8_t transmissions) {
	LOGd("sendTrackedDeviceHeartbeat id=%u location=%u TTL=%u",
		 packet->deviceId,
		 packet->locationId,
		 packet->ttlMinutes);
...
cs_ret_code_t MeshMsgSender::sendTrackedDeviceListSize(
		const cs_mesh_model_msg_device_list_size_t* packet, uint8_t transmissions) {
...
cs_ret_code_t MeshMsgSender::handleSendMeshCommand(
	...
	switch (command->controlCommand.type) {
		...
		case CTRL_CMD_STATE_SET: {
			...
			memcpy(meshMsg + sizeof(*meshStateHeader),
				   command->controlCommand.data + stateHeaderSize,
				   statePayloadSize);
		...
		default: {
			...
			cs_mesh_model_msg_ctrl_cmd_header_ext_t* meshMsgHeader =
					reinterpret_cast<cs_mesh_model_msg_ctrl_cmd_header_ext_t*>(meshMsg);
...
void MeshMsgSender::handleEvent(event_t& event) {
	switch (event.type) {
		...
		case CS_TYPE::CMD_SEND_MESH_MSG_SET_BEHAVIOUR_SETTINGS: {
			TYPIFY(CMD_SEND_MESH_MSG_SET_BEHAVIOUR_SETTINGS)* packet =
					(TYPIFY(CMD_SEND_MESH_MSG_SET_BEHAVIOUR_SETTINGS)*)event.data;
			event.result.returnCode = sendBehaviourSettings(packet);
			break;
		}
		case CS_TYPE::CMD_SEND_MESH_MSG_PROFILE_LOCATION: {
			TYPIFY(CMD_SEND_MESH_MSG_PROFILE_LOCATION)* packet =
					(TYPIFY(CMD_SEND_MESH_MSG_PROFILE_LOCATION)*)event.data;
			event.result.returnCode = sendProfileLocation(packet);
			break;
		}
		case CS_TYPE::CMD_SEND_MESH_MSG_TRACKED_DEVICE_REGISTER: {
			TYPIFY(CMD_SEND_MESH_MSG_TRACKED_DEVICE_REGISTER)* packet =
					(TYPIFY(CMD_SEND_MESH_MSG_TRACKED_DEVICE_REGISTER)*)event.data;
			event.result.returnCode = sendTrackedDeviceRegister(packet);
			break;
		}
		case CS_TYPE::CMD_SEND_MESH_MSG_TRACKED_DEVICE_TOKEN: {
			TYPIFY(CMD_SEND_MESH_MSG_TRACKED_DEVICE_TOKEN)* packet =
					(TYPIFY(CMD_SEND_MESH_MSG_TRACKED_DEVICE_TOKEN)*)event.data;
			event.result.returnCode = sendTrackedDeviceToken(packet);
			break;
		}
		case CS_TYPE::CMD_SEND_MESH_MSG_TRACKED_DEVICE_HEARTBEAT: {
			TYPIFY(CMD_SEND_MESH_MSG_TRACKED_DEVICE_HEARTBEAT)* packet =
					(TYPIFY(CMD_SEND_MESH_MSG_TRACKED_DEVICE_HEARTBEAT)*)event.data;
			event.result.returnCode = sendTrackedDeviceHeartbeat(packet);
			break;
		}
		case CS_TYPE::CMD_SEND_MESH_MSG_TRACKED_DEVICE_LIST_SIZE: {
			TYPIFY(CMD_SEND_MESH_MSG_TRACKED_DEVICE_LIST_SIZE)* packet =
					(TYPIFY(CMD_SEND_MESH_MSG_TRACKED_DEVICE_LIST_SIZE)*)event.data;

...
hardware_version() {
	...
	cs_info "Use jlink to run hardware_version.script (reads single memory location)"
	if [ -z $serial_num ]; then
		cs_log "$JLINK -Device $DEVICE -If SWD $TEMP_DIR/hardware_version.script -ExitonError 1"
		$JLINK -Device $DEVICE -speed 100 -If SWD $TEMP_DIR/hardware_version.script -ExitonError 1
	else
		cs_log "$JLINK -Device $DEVICE -SelectEmuBySN $serial_num -If SWD $TEMP_DIR/hardware_version.script -ExitonError 1"
	...
	cs_log
	cs_log "This would mean the hardware version is AAB0 (read hex numbers from back to front)"
	cs_log "See FICR in the product specification"
	cs_log "B0 are the first 2 characters of the last 3 characters in the errata docs"
}

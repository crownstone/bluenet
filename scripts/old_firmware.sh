...
unit-test-host() {
  ...
  result=$?
  checkError $result "oo Error building firmware"
  cd "$path"
}
...
writeHardwareVersion() {
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ] && [ -n "$HARDWARE_BOARD_INT" ]; then
      ...
      result=$?
      checkError $result "Error writing hardware version"
    else
...
upload() {
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ $use_combined ]; then
      ...
      result=$?
      checkError $result "Error with uploading firmware"
    else
...
debug() {
  ...
  result=$?
  checkError $result "Error debugging firmware"
}

debugbl() {
  ${path}/_debug.sh $BLUENET_BIN_DIR/bootloader.elf $serial_num $gdb_port
  result=$?
  checkError $result "Error debugging bootloader"
}
...
clean() {
  ...
  result=$?
  checkError $result "Error cleaning up"
  cs_info "Cleaned up!"
}
...
uploadBootloader() {
  ...
  if [ $? -eq 0 ]; then
    ...
    result=$?
    checkError $result "Error uploading bootloader"
...
bootloader() {
  ...
  if [ $? -eq 0 ]; then
    ...
    result=$?
    checkError $result "Error marking app valid"
  fi
}
...
if [ $? -eq 0 ]; then
  ...
  result=$?
  checkError $result "Error marking app invalid, note INVALID (only a bootloader is flashed, no valid app)"
fi
}
...
release() {
  ...
  make release
  result=$?
  checkError $result "Failed to build release"
  # result=$?
  cd $path
  # return $result
}

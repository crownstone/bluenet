...
if [ -z $SERIAL_NUM ]; then
	cs_log "$JLINK -Device $DEVICE -speed 4000 -If SWD $TEMP_DIR/writebyte.script -ExitonError 1"
	$JLINK -Device $DEVICE -speed 4000 -If SWD $TEMP_DIR/writebyte.script -ExitonError 1
else
	cs_log "$JLINK -Device $DEVICE -speed 4000 -SelectEmuBySN $SERIAL_NUM -If SWD $TEMP_DIR/writebyte.script -ExitonError 1"
